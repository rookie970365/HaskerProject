...
from .fixtures import DataSetupMixin, TEST_USER, TEST_PASSWORD


class TestAskView(DataSetupMixin, TestCase):
...
class TestQuestionDetail(DataSetupMixin, TestCase):
...
class TestQuestionSearch(DataSetupMixin, TestCase):
...
class TestVote(DataSetupMixin, TestCase):
...
class TestAnswerMark(DataSetupMixin, TestCase):
    def test_GET_unauthorized(self):
        client = Client()
        response = client.get(
    ...
    def test_valid(self):
        state_before = self.answer_1.is_accepted
        client = Client()
        client.login(username=TEST_USER, password=TEST_PASSWORD)
        response = client.post(
            reverse(f"answer_mark", args=(self.answer_1.pk,)),
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json()["accepted"], not state_before)
        self.answer_1.refresh_from_db()
        self.assertEqual(self.answer_1.is_accepted, not state_before)
