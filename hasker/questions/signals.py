...
def answer_created(sender, instance, created, raw, *args, **kwargs):
    ...
    if created and not raw:
        ...
        logger.debug(
            f"Number of answers has been increased for {instance.question.pk}"
        )
...
def answer_deleted(sender, instance, *args, **kwargs):
    ...
    logger.debug(
        f"Number of answers has been decreased for {instance.question.pk}."
        f"Answer has been deleted"
    )
...
def vote_created(sender, instance, created, raw, *args, **kwargs):
    ...
    if created:
        ...
        logger.debug(
            f"Number of votes / rating have been changed for {post_model} ({instance.pk})"
        )
    else:
        model = type(instance)
